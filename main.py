...
async def start_auto_packet(key, iv, region):
    # Correct packet for starting the match: {1:9, 2:{}}
    fields = {1: 9, 2: {}}
    if region.lower() == "ind":
        packet_type = '0514'
    elif region.lower() == "bd":
...
[FFFFFF]rmgamingindsmove , furiousslam , moonflip , wigglewalk
    ...
    for i in range(SPAM_REQUESTS):
        try:
            ...
            fields = {
                ...
                2: {
                    1: int(Rm),
                    2: "iG:[C][B][FF0000]rmgamingind CODEX",
                    3: {2: 1, 3: 1},
                    4: 330,
                    5: 6000,
                    6: 201,
...
async def reject_spam_loop(target_uid, key, iv):
    ...
    while reject_spam_running and count < max_spam:
        try:
            packet1 = await rmgamingind1(target_uid, key, iv)
            packet2 = await rmgamingind(target_uid, key, iv)
...
async def handle_title_final(inPuTMsG, uid, chat_id, key, iv, region, chat_type=0):
    ...
    try:
        if not whisper_writer:
            return
        title_packet = await convert_kyro_to_your_system(target_uid, chat_id, key, iv, "rmgamingind", selected_title)
        if title_packet and whisper_writer:
            whisper_writer.write(title_packet)
            await whisper_writer.drain()
    except Exception:
            ...
            while True:
                ...
                if data_hex.startswith("0500") and insquad is None and joining_team == False:
                    try:
                        ...
                        insquad = True
                        try:
                            welcome_message = """[B][C][FFFFFF]Welcome to rmgamingind Bot
...
[B][C][FFFFFF]Telegram : @rmgamingind
[B][C][FFFFFF]Contact : @rmgamingind
...
[B][C][FFFFFF]Status: Online & Active"""
                ...
                if data2.hex().startswith('0500') and len(data2.hex()) > 1000:
                    try:
                        ...
                        message = """[B][C][FFFFFF]Welcome to rmgamingind Bot
...
[FFFFFF]• rampage • cannibal • rmgamingind
...
"""
                        ...
                        if inPuTMsG.strip() == "/admin":
                            admin_message = """
[C][B][FFD700]    ✦─────────✦
       💧 rmgamingind BOT ADMIN
    ✦─────────✦

[C][B][00FF00]👑 BOT DEVELOPER: 
[C][B][FFFFFF]rmgamingind CODEX
...
[FFFFFF]
2[C]8[C]4[C]3[C]8[C]5[C]2[C]8[C]5[C]0[C]
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 3:
                                ...
                                await dl_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                try:
                                    success = await Room_Spam(target_uid, room_id, "rmgamingind CODEX", key, iv)
                                    if success:
                        ...
                        if inPuTMsG.strip().startswith('/sticker'):
                            packet = await send_sticker(uid, chat_id, key, iv, nickname="rmgamingindxBOT")
                            if packet:
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop evo_c':
                            if evo_custom_spam_task and not evo_custom_spam_task.done():
                                ...
                                await dl_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        if inPuTMsG.strip().lower() == "rmgamingind bhai mera uid do":
